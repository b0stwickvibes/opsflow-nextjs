...
// Static industry configuration - moved outside component to prevent re-creation
...
export function KPIShowcase({ industry = "restaurants", variant = 'outline', energy = 'balanced', leadKey }: KPIShowcaseProps) {
  ...
  const config = useMemo(() => industryConfig[industry], [industry]);
  const finalStats = useMemo(() => config.finalStats, [config]);

  const [stats, setStats] = useState(() => finalStats);

  // Update stats when industry changes
  useEffect(() => {
    setStats(finalStats);
  }, [industry]); // Use industry as dependency instead of finalStats
  ...
  return (
    <section className={`py-32 ${(() => { switch (industry) { case 'restaurants': return 'accent-orange'; case 'bars': return 'accent-purple'; case 'coffee': return 'accent-amber'; case 'hotels': return 'accent-blue'; default: return ''; } })()}`}>
      <div className="container flex justify-center">
        <div className="flex w-full flex-col justify-between gap-8 lg:flex-row">
          <div className="w-full lg:w-1/3">
            <h1 className="text-display-2xl w-full  font-bold lg:text-6xl">
              {config.title}
            </h1>
            <p className="enterprise-body my-4  tracking-tight text-muted-foreground">
              {config.subtitle}
            </p>
            <Button
              onClick={handleGetStarted}
              className="group text-md mt-10 flex w-fit items-center justify-center gap-2 rounded-full px-6 py-3 tracking-tight text-primary-foreground bg-brand-gradient hover:scale-105 transition-all"
            >
            ...
            <div className="mt-auto mb-10 grid w-full grid-cols-2 gap-8">
              {config.kpis.map((kpi, index) => {
                ...
                return (
                  ...
                  >
                    <h2 
                      className={`enterprise-headline text-3xl font-bold ${getValueClasses(appliedVariant)}`}
                    >
                      <NumberFlow
                        value={
                          showMonthlyStats
