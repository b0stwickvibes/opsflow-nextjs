...
const INDUSTRY_EARNINGS_CONTENT = {
  restaurant: {
    title: "Boost your restaurant revenue without expanding locations",
    subtitle: "revenue",
    features: [
      "Average 15% profit increase",
      "Access kitchen analytics daily",
      "Recommendations by food service experts",
    ],
    ctaText: "Start Optimizing",
    mockupSrc: "https://deifkwefumgah.cloudfront.net/shadcnblocks/block/placeholder-dark-7-tall.svg",
  ...
  bar: {
    ...
    features: [
      ...
      "Access sales insights hourly",
      "Strategies from bar industry specialists",
    ],
  ...
  cafe: {
    ...
    subtitle: "income",
    features: [
      "Average 12% profit improvement",
      "Access performance metrics twice daily",
      "Insights from café operations experts",
    ],
...
const EarningHero = ({ 
  ...
  const showMobileApp = useFeatureFlag('earningHeroMobileApp', true);
  const showAnimatedSVG = useFeatureFlag('heroAnimatedAccent', true);
  
  usePageView(`${industry}_earning_hero`, { 
    variant,
    industry,
    role
  });
  ...
  // Permission checking
  const canStartOptimization = usePermission('OPTIMIZATION_START_ACCESS');

  const content = INDUSTRY_EARNINGS_CONTENT[industry];
  ...
  return (
    <section className={cn("bg-gradient-to-br", content.bgColor, "py-12 font-sans md:py-20")}>
      <div className="container flex flex-col items-center gap-8 lg:flex-row">
        ...
        <div className="flex flex-col gap-10 animate-fade-in-up">
          ...
          <div className={cn("inline-flex items-center gap-2 self-start px-3 py-1 rounded-full border", colors.primary)}>
            ...
            <span className="text-sm font-medium capitalize">{industry} Growth</span>
          </div>
            ...
            <span className="relative inline-block">
              ...
              {showAnimatedSVG && (
                <svg
                  width="240"
                  className={cn(
                    "pointer-events-none absolute top-1/2 left-1/2 hidden -translate-x-1/2 -translate-y-1/2 transform md:block animate-pulse-subtle",
                    colors.accent
                  )}
                  viewBox="0 0 191 75"
                  fill="none"
          ...
          <ul className="flex flex-col gap-4 animate-fade-in-up animation-delay-200">
            {content.features.map((feature, index) => (
              <li key={index} className="flex items-center gap-3">
                <div className="flex h-8 w-8 rounded-full bg-accent border">
                  <Check
                    className="m-auto size-5 stroke-foreground"
                    strokeWidth={3}
                  />
                </div>
          ...
          <Button
            asChild
            className={cn(
              "flex h-fit w-full items-center justify-center gap-2 rounded-3xl px-8 py-4 font-medium text-lg md:w-fit shadow-lg hover:shadow-xl transition-all duration-200 animate-fade-in-up animation-delay-300",
              colors.bg,
              colors.hover,
              !canStartOptimization && "opacity-75 cursor-not-allowed"
            )}
            disabled={!canStartOptimization}
          >
            <a href="#" aria-label={`${content.ctaText} for ${industry}`}>
              <div>{content.ctaText}</div>
        ...
        {showMobileApp && (
          <div className="w-full shrink-0 lg:w-1/2 animate-fade-in-up animation-delay-100">
            <div className="relative mx-auto h-[27.5rem] w-full max-w-[36.5rem] overflow-hidden rounded-3xl bg-gradient-to-br from-muted to-muted/50 bg-cover bg-center bg-no-repeat sm:h-[39.5rem] shadow-2xl">
              ...
              <div className="relative mx-auto mt-20 w-[12.5rem] sm:w-[18.75rem]">
                ...
                {variant === 'detailed' && (
                  <div className="absolute -right-4 top-16 z-30 bg-background/90 backdrop-blur-sm border rounded-lg p-3 shadow-lg animate-slide-in-right animation-delay-600">
                    <div className="flex items-center gap-2">
                      <TrendingUp className={cn("size-4", colors.primary)} />
                      <div className="text-xs font-medium">
                        +{content.features[0].match(/\d+/)?.[0]}% Growth
                      </div>
                    </div>
                  </div>
                )}
                ...
                {variant === 'detailed' && (
                  <div className="absolute -left-4 bottom-20 z-30 bg-background/90 backdrop-blur-sm border rounded-lg p-3 shadow-lg animate-slide-in-left animation-delay-700">
                    <div className="flex items-center gap-2">
                      <DollarSign className={cn("size-4", colors.primary)} />
                      <div className="text-xs font-medium">
                        Real-time Analytics
                      </div>
                    </div>
