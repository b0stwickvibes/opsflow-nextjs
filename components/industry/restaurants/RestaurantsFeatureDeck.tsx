...
/**
 * RestaurantsFeatureDeck Component
 *
 * Displays core restaurant management features in a 3-column grid layout.
 * Features clean Clerk.com styling with BLUE checkbox accents (restaurants theme).
 *
 * ENTERPRISE DESIGN STANDARDS:
 * - Section-compatible: No internal section/container wrappers
 * - Uses OKLCH color tokens with blue checkboxes for restaurants theme
 * - Beautiful hover effects with shadow transitions
 * - Adapts to parent accent theme (blue for restaurants via .accent-blue)
 *
 * USAGE:
 * <Section background="muted" padding="lg">
 *   <SectionContent maxWidth="5xl">
 *     <RestaurantsFeatureDeck {...props} />
 *   </SectionContent>
 * </Section>
 */
...
export interface RestaurantsFeatureDeckProps {
  title: string;
  description: string;
  badge?: string;
  features: Feature[];
...
export function RestaurantsFeatureDeck({
  title,
  description,
  badge,
  features,
  ...
  return (
    <div className={className}>
      ...
      {/* Features Grid - Clerk.com Clean Style with BLUE checkboxes */}
          ...
          return (
            ...
            >
              <div className="bg-background border border-border rounded-3xl p-8 h-full group hover:border-primary/20 hover:shadow-xl hover:shadow-primary/5 transition-all duration-500">
                ...
                {/* Benefits - Clean List with BLUE checkboxes (restaurants theme) */}
                <div className="space-y-3 mb-6">
                  {feature.benefits.slice(0, 3).map((benefit, benefitIndex) => (
                    <div key={benefitIndex} className="flex items-start gap-3">
                      {/* BLUE checkbox - signature restaurants theme element */}
